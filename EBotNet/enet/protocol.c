...
static ENetPeer*
enet_protocol_handle_connectU(ENetHost* host, ENetProtocolHeaderUbisoft* header, ENetProtocol* command)
{
    enet_uint8 incomingSessionID, outgoingSessionID;
    enet_uint32 mtu, windowSize;
    ENetChannel* channel;
...
{
    size_t useNew = (host->protocolRev == 1);
    ENetProtocolHeader* header = NULL;
    ENetProtocolHeaderUbisoft* newHeader = NULL;
    ...
    if (host->receivedDataLength < (size_t) & ((ENetProtocolHeader*)0)->sentTime && !useNew)
        return 0;
    if (host->receivedDataLength < (size_t) & ((ENetProtocolHeaderUbisoft*)0)->sentTime && useNew)
        return 0;

    if (useNew) newHeader = (ENetProtocolHeaderUbisoft*)host->receivedData;
    ...
    if (useNew) headerSize = (flags & ENET_PROTOCOL_HEADER_FLAG_SENT_TIME ? sizeof(ENetProtocolHeaderUbisoft) : (size_t) & ((ENetProtocolHeaderUbisoft*)0)->sentTime);
...
enet_protocol_receive_incoming_commands(ENetHost* host, ENetEvent* event)
{
    ...
    for (packets = 0; packets < 256; ++packets)
    {
        ...
        int requiredLength = (host->relayAddress.port == 0) ? 0 : sizeof(ENetSocksHeader);
        if (receivedLength < requiredLength)
            return -1;

        if (host->relayAddress.port != 0 && buffer.dataLength < requiredLength)
            return -1;

        host->receivedData = host->packetData[0];
        host->receivedDataLength = receivedLength;

        if (host->relayAddress.port != 0)
        {
            host->receivedData += sizeof(ENetSocksHeader);
            host->receivedDataLength -= sizeof(ENetSocksHeader);
        }
...
{
    size_t useNew = (host->protocolRev == 2);
    size_t packetSize = useNew ? sizeof(ENetProtocolHeaderUbisoft) : sizeof(ENetProtocolHeader);

    enet_uint8 payload[sizeof(ENetSocksHeader) + sizeof(ENetProtocolHeaderUbisoft) + sizeof(enet_uint32)];
    ...
    if (useNew) 
    {
        enet_uint16 port = (host->relayAddress.port == 0) ? 
            host->peers->address.port : 
            host->relayAddress.port;
    ...
    while (host->continueSending)
            ...
            ++currentPeer)
        {
            ...
            if (host->compressor.context != NULL && host->compressor.compress != NULL)
            {
                ...
                if (useNew) originalSize = host->packetSize - sizeof(ENetProtocolHeaderUbisoft);
                size_t compressedSize = host->compressor.compress(host->compressor.context,
            ...
            if (host->relayAddress.port != 0)
            {
                ENetSocksHeader socksHeader;
                socksHeader.Version = 5;
                socksHeader.AddrType = 1;
                socksHeader.DestPort = htons(host->relayAddress.port);
                socksHeader.IPv4 = host->relayAddress.host;
                socksHeader.Fragment = 0;
                socksHeader.Reserved = 0;

                *(ENetSocksHeader*)payload = socksHeader;

                memcpy(payload + sizeof(ENetSocksHeader), host->buffers->data, host->buffers->dataLength);
                host->buffers->dataLength += sizeof(ENetSocksHeader);
                host->buffers->data = payload;
            }
...
void
enet_host_udp_tunnel(ENetHost* host, const ENetAddress* address)
{
    host->relayAddress = *address;
}
